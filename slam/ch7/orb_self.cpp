...
#include <string>
#include <nmmintrin.h>   //使用SSE指令集进行加速
#include <chrono>
using namespace std;

// global variables
string first_file = "/home/yikang/cppSpace/slam/ch7/3.png";
string second_file = "/home/yikang/cppSpace/slam/ch7/4.png";
...
int main(int argc, char **argv) {
  ...
  // load image
  cv::Mat first_image = cv::imread(first_file, 0);   //flag = 0,读入单通道图像
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
  ...
  int bad_points = 0;
  // if a keypoint goes outside the image boundary (8 pixels), descriptors will not be computed and will be left as empty
  for (auto &kp: keypoints) {
    if (kp.pt.x < half_boundary || kp.pt.y < half_boundary ||
        kp.pt.x >= img.cols - half_boundary || kp.pt.y >= img.rows - half_boundary) {
      // outside
      bad_points++;
      descriptors.push_back({});  //压入空白点
      continue;
    }

    //计算灰度质心位置，进而计算关键点方向信息
    float m01 = 0, m10 = 0;
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
  ...
  for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
    ...
    for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
      ...
      int distance = 0;
      for (int k = 0; k < 8; k++) {
        distance += _mm_popcnt_u32(desc1[i1][k] ^ desc2[i2][k]);  // _mm_popcnt_u32是SSE指令集中的函数
      }
